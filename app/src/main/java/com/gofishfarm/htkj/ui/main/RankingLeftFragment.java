...
import com.gofishfarm.htkj.presenter.main.RankingLeftPresenter;
import com.gofishfarm.htkj.ui.login.LoginActivity;
import com.gofishfarm.htkj.ui.main.adapter.RankleftAdapter;
 ...
 * @time 2019/1/5
 */
public class RankingLeftFragment extends BaseFragment<RankingLeftView, RankingLeftPresenter> implements RankingLeftView {
    ...
    RankleftAdapter adapter;
    List<RankLfetBean.InfoBean> mInfoBean = new ArrayList<>();
    private boolean mOne = false;//判断是否第一次进入页面

    @Inject
    RankingLeftPresenter mAttentionPresenter;

    public static RankingLeftFragment newInstance() {
        return new RankingLeftFragment();
    }

    @Override
    public RankingLeftPresenter createPresenter() {
        return this.mAttentionPresenter;
    }

    @Override
    public RankingLeftView createView() {
        return this;
    }
...
//            showDialog("正在加载");
    ...
    private void initData() {
        mAttentionPresenter.getRankLftBeanData(SharedUtils.getInstance().getString(ConfigApi.AUTHORIZATION, ""));
    }
    ...
    private void initAdapter() {
        ...
        adapter = new RankleftAdapter(R.layout.item_rank, mInfoBean);
...
//                ToastUtils.show("" + position);
    ...
    private void initLayout(View view) {
        ...
        tv_fast_most.setText(getString(R.string.fish_num));
    ...
    @Override
    public void onRankLeftBeanDataCallback(RankLfetBean mRankLfetBean) {
        dismissDialog();
        mOne = false;
        if (null == mRankLfetBean) {
            return;
        }
        setData(mRankLfetBean);
    }

    private void setData(RankLfetBean mRankLfetBean) {
        if (null != mInfoBean) {
            mInfoBean.clear();
        }
        mInfoBean.addAll(mRankLfetBean.getInfo());
        initHeadInfo();
