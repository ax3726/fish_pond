 ...
 * @time 2018/7/30
 */
public abstract class BaseSubscriber<T> extends ResourceSubscriber<BaseBean> {
    private String mMsg;
    ...
    @Override
    public void onNext(com.gofishfarm.htkj.bean.BaseBean paramBean) {
        if (this.mView == null) {
            return;
        }
        if (paramBean.getCode() == 200) {
            this.mView.complete(paramBean.getMessage());
            onSuccess(paramBean);
        } else if (paramBean.getCode() == 401) {
            this.mView.complete("");
            ...
            this.mView.showError(paramBean.getMessage(), paramBean.getCode());
        }
    }

    public abstract void onSuccess(com.gofishfarm.htkj.bean.BaseBean param);
...
//            this.mView.showError(this.mMsg, 501);
//        } else if ((paramThrowable instanceof ApiException)) {
//            this.mView.showError(paramThrowable.toString(), 501);
//        } else if ((paramThrowable instanceof SocketTimeoutException)) {
//            this.mView.showError("服务器响应超时", 501);
//        } else if ((paramThrowable instanceof HttpException)) {
//            this.mView.showError("数据加载失败", 501);
//        } else {
//            this.mView.showError("未知错误", 501);
//        }
    }
